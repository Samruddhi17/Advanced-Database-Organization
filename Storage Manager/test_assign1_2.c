...
testSinglePageContent(void)
{
  ...
  // write string to current position of file.
  TEST_CHECK(writeCurrentBlock(&fh, ph));
  printf("writing Current block\n");
  
  // write string to first page of file.
  TEST_CHECK(writeBlock(1, &fh, ph));
  printf("writing Intermediate Block 1 block\n");

  // write string to current position of file.
  TEST_CHECK(writeCurrentBlock(&fh, ph));
  printf("writing Intermediate Block 1_current block\n");
  ...
  fh.curPagePos = 5000;//moving current page position to second block.
